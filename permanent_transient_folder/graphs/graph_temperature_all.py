...
caminho_da_pasta_resultados = "permanent_transient_folder/output/"
padrao_arquivos = "data_*" # Padrão para encontrar os arquivos de dados

# --- 2. LÓGICA PRINCIPAL ---
caminho_dados_limpos = "permanent_transient_folder/data/direct_problem_9_80_2e+04.npz"
try:
    ...
    for i, arquivo_h5 in enumerate(lista_de_arquivos):
        ...
        try:
            with h5py.File(arquivo_h5, 'r') as hf:
                ...
                indices_sensores = np.arange(len(T_medida[-1]))
                
                # --- Plot das temperaturas Medida e Estimada ---
                ax.plot(indices_sensores, T_medida[-1], 'o-', markersize=3, color='lightskyblue', label='T Medida (com ruído)')
                ax.plot(indices_sensores, T_estimada[-1], 'o-', markersize=3, color='darkblue', label='T Estimada (final)')
