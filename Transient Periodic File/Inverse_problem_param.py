...
def calculate_difference(args: Tuple[int, np.ndarray, np.ndarray, float, float, float, tuple]) -> Tuple[float, int]:
    ...
    q_modified = heat_flux_approximation(parameters_modified, num_theta, max_time_steps)  # Current heat flux approximation
...
def compute_differences(parameters: np.ndarray, T_real: np.ndarray, lambda_regul: float,
    ...
    q = heat_flux_approximation(parameters, num_theta, max_time_steps)  # Current heat flux approximation
    ...
    gradiente = np.zeros_like(parameters, dtype=np.float64)
...
def heat_flux_approximation(parameters: np.ndarray, ntheta: int, experimental_time) -> np.ndarray:
    """
    ...
    D = parameters[2 * N + 1]
    E = parameters[2 * N + 2 : 3 * N + 2]
    F = parameters[3 * N + 2 : 4 * N + 2]
    ...
    q_theta = A + sum_cos_theta + sum_sin_theta
...
if __name__ == '__main__':
    ...
    radial_size = 9
    angular_size = 80
    experiment_time = 1000 # 12 hours in seconds
    num_sensors = 20
