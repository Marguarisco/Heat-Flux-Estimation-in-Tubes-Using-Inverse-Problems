...
def calculate_difference(args: Tuple[int, np.ndarray, np.ndarray, float, float, float, tuple]) -> Tuple[float, int]:
    ...
    q_modified = heat_flux_aproximation(parameters_modified, num_theta)  # Current heat flux approximation
...
def compute_differences(parameters: np.ndarray, T_real: np.ndarray, lambda_regul: float,
    ...
    q = heat_flux_aproximation(parameters, num_theta)  # Current heat flux approximation
...
def heat_flux_aproximation(parameters: np.ndarray, ntheta: int) -> np.ndarray:
    """
    Approximates the heat flux based on the parameters.

    Parameters:
    parameters (np.ndarray): Parameters [A, B, C].
    ...
    q = np.array([A * (t) ** 2 + B * t + C for t in theta])'''
    ...
    internal =  w * (theta + np.pi) / (2 * np.pi)
...
if __name__ == '__main__':
    ...
    radial_size = 9
    angular_size = 80
    experiment_time = 1200 # 12 hours in seconds
    num_sensors = 20
